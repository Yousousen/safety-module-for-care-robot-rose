...
/********************************** INTERFACE *********************************/
#ifndef ISAFETYCHECK_HH
#define ISAFETYCHECK_HH
...
struct ISafetyCheck
{
  ...
  struct
  {
    std::function< ::Behavior::type()> do_check;
  } in;
  ...
  dzn::port::meta meta;
  inline ISafetyCheck(const dzn::port::meta& m) : meta(m) {}

  void check_bindings() const
  {
    if (! in.do_check) throw dzn::binding_error(meta, "in.do_check");


  }
};

inline void connect (ISafetyCheck& provided, ISafetyCheck& required)
{
